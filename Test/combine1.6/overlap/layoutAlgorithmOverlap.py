...
# overlapNode stands for an overlapNode object
# startAngle stands for the angle which the drawing starts
# situationNum stands for the number of drawing pattern
...
# startAngle stands for the angle which the drawing starts
# situationNum stands for the number of drawing pattern
# radiusList[] -> Main difference from PATTERN 1!!!
...
# overlapPair stands for a pair of overlapNode object
# initialCenter stands for the initial drawing center of this paired nodes
...
# Calculate the positions of a group overlapNode in pattern 2
# overlapPair stands for a pair of overlapNode object
# initialCenter stands for the initial drawing center of this paired nodes
def calGroupPosition2(overlapGroup, initialCenter):
    ...
    if len(overlapNodeList) == 2:
        ...
        for nextOverlapNode in overlapNodeList[2::]:
            ...
            overlapPosition = connectionName.find(nextOverlapPartName[::-1])
            nextOverlapNode.getOverlapPart().setName(nextOverlapNode.getOverlapPart().getName()[::-1])
            ...
            moveDistance = math.sqrt(0.7 * 0.7 - 0.25 * 0.25) + 1 + 0.18 * 4 * (overlapPosition - overlapNode1_OverlapPartNameLength) + nextOverlapNode_OverlapPartLength_half
...
def ifIntersect(ls1, ls2):
    ...
    # If ls1 parallel to y-axis but ls2 not
        ...
        if (ls1_yRange[0] <= y_intersect <= ls1_yRange[1] and ls2_yRange[0] <= y_intersect <= ls2_yRange[1]) and (ls1_xRange[0] <= ls1[0, 0] <= ls1_xRange[1] and ls2_xRange[0] <= ls1[0, 0] <= ls2_xRange[1]):
            return True
        else:
            return False
    # If ls2 parallel to y-axis but ls2 not
        ...
        if (ls1_yRange[0] <= y_intersect <= ls1_yRange[1] and ls2_yRange[0] <= y_intersect <= ls2_yRange[1]) and (ls1_xRange[0] <= ls2[0, 0] <= ls1_xRange[1] and ls2_xRange[0] <= ls2[0, 0] <= ls2_xRange[1]):
            return True
        else:
            return False
    # If both ls1 and ls2 not parallel to the y-axis
        ...
        if k1 == k2:
            if b1 == b2:
                if (ls1_yRange[0] <= ls2[0, 1] <= ls1_yRange[1] or ls1_yRange[0] <= ls2[1, 1] <= ls1_yRange[1] or ls2_yRange[0] <= ls1[0, 1] <= ls2_yRange[1] or ls2_yRange[0] <= ls1[1, 1] <= ls2_yRange[1]) and (ls1_xRange[0] <= ls2[0, 0] <= ls1_xRange[1] or ls1_xRange[0] <= ls2[1, 0] <= ls1_xRange[1] or ls2_xRange[0] <= ls1[0, 0] <= ls2_xRange[1] or ls2_xRange[0] <= ls1[1, 0] <= ls2_xRange[1]):
                    return True
                else:
                    return False
            else:
...
# Calculate the edge positions of two nodes
...
# graph stands for a graph object
# patternNum stands for the drawing pattern number
                    ...
                    for nextNode in nextNodeList:
                        if sorted(node.getName()) == sorted(nextNode.getName()):
                            ...
                            if patternNum == 2:
                                ...
                                nextConnectRemain = nextNode.getName().lstrip(nextOverlapNode.getOverlapPart().getName())
