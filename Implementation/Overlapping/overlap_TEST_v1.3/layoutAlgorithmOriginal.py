...
def calLayout(graph, patternNum):
    ...
    for node in nodeList:
        if isinstance(node, OverlapNode):
            innerNodeList = node.getNodesContain()
            for innerNode in innerNodeList:
                allNodeList.append(innerNode)
        elif isinstance(node, OverlapNode2):
            innerNodeList = node.getNodesContain()
            for innerNode in innerNodeList:
                allNodeList.append(innerNode)
    ...
    i = 0
    rotateRate = 360 / len(allNodeList)
    radius = len(allNodeList) / 30 * 15

    for node in allNodeList:
        rotateAngle = rotateRate * i
        center = [0 + radius * math.cos(math.radians(rotateAngle)), 0 + radius * math.sin(math.radians(rotateAngle))]
...
def checkNodeOverlap(node1, node2):
    if isinstance(node1, Node) and isinstance(node2, Node):
        return ifOverlap(node1.getNodeCorners(), node2.getNodeCorners())
