...
def drawLiteral(node):
    ...
    # Calculate the draw position of the 1st literal
    drawPosition = [centerX - literalsLength / 2 * math.cos(math.radians(
        rotateAngle)), centerY - literalsLength / 2 * math.sin(math.radians(rotateAngle))]
    # Insert blank spaces as intervals
    drawLiterals = []
    for i in literals:
        drawLiterals.append(i)
    ...
    for i in drawLiterals:
        ...
        if(i.isalpha()):
            # the position is calculated at the first time so no need to calculate again.
            global first
            if(not first):
            ...
            num = True
            plt.text(drawPosition[0] - horizonMove,
                     drawPosition[1] - verticalMove, i, fontsize=10)
    first = True
...
def drawRectangle(node, axis):
    ...
    tr = matplotlib.transforms.Affine2D().rotate_deg_around(
        centerX, centerY, rotateAngle)
    t = tr + ts
    rect1 = matplotlib.patches.Rectangle((centerX - nodeWidth / 2, centerY-0.25), nodeWidth,
                                         nodeHeight, linewidth=1, edgecolor='black', facecolor='none', transform=t)
...
def drawEdge(node1, node2):
    ...
    plt.plot([drawPointPair[0][0], drawPointPair[1][0]],
             [drawPointPair[0][1], drawPointPair[1][1]])
...
def drawArcCombo(center, radius, rotateAngle, node, axis):
    ...
    nodeCenter = [center[0] + calLength * math.cos(math.radians(
    ...
    point1 = [nodeCenter[0] - callength2 * math.cos(math.radians(
        calAngle1)), nodeCenter[1] - callength2 * math.sin(math.radians(calAngle1))]
    point2 = [nodeCenter[0] - callength2 * math.cos(math.radians(
        calAngle2)), nodeCenter[1] - callength2 * math.sin(math.radians(calAngle2))]
...
def calDistance(point1, point2):
    ...
    distance = math.sqrt((point1[0]-point2[0]) * (point1[0] -
