...
def drawLiteral(literals, center, rotateAngle, plt):
    ...
    drawPosition = [center[0] - literalsLength / 2 * math.cos(math.radians(
        rotateAngle)), center[1] - literalsLength / 2 * math.sin(math.radians(rotateAngle))]
    ...
    for i in drawLiterals:
        plt.text(drawPosition[0] - horizonMove,
                 drawPosition[1] - verticalMove, i, fontsize=16)
        drawPosition = [drawPosition[0] + (literalInterval*2) * math.cos(math.radians(
            rotateAngle)), drawPosition[1] + (literalInterval*2) * math.sin(math.radians(rotateAngle))]
...
def drawRectangle(literals, centerR, rotateAngle, plt):
    ...
    tr = matplotlib.transforms.Affine2D().rotate_deg_around(
        centerR[0], centerR[1], rotateAngle)
    t = tr + ts
    rect1 = matplotlib.patches.Rectangle((centerR[0] - rectLength / 2, centerR[1]-0.25),
                                         rectLength, 0.5, linewidth=1, edgecolor='black', facecolor='none', transform=t)
    ax1.add_patch(rect1)
...
def drawArc(center, radius, rotateAngle):
    ...
    arc1 = matplotlib.patches.Arc(
        center, radius * 2, radius * 2, theta1=endAngle, theta2=startAngle, color="black")
    ax1.add_patch(arc1)
...
def drawArcCombo(center, radius, rotateAngle, remainLiterals):
    ...
    centerR = [center[0] + calLength * math.cos(math.radians(
        rotateAngle)), center[1] + calLength * math.sin(math.radians(rotateAngle))]
    ...
    point1 = [centerR[0] - callength2 * math.cos(math.radians(
        calAngle1)), centerR[1] - callength2 * math.sin(math.radians(calAngle1))]
    point2 = [centerR[0] - callength2 * math.cos(math.radians(
        calAngle2)), centerR[1] - callength2 * math.sin(math.radians(calAngle2))]
...
def drawPattern2(overlapLiterals, remainLiteralList, center):
    ...
    for remainLiterals in remainLiteralList:
        ...
        if (i < len(remainLiteralList) - 1):
            ...
            centers2 = [center[0] + calLength2 * math.cos(math.radians(
                rotateAngle2)), center[1] + calLength2 * math.sin(math.radians(rotateAngle2))]
            nextDots = drawDot(
                remainLiteralList[i + 1], centers2, rotateAngle2, plt)
            plt.plot([thisDots[1][0], nextDots[0][0]],
                     [thisDots[1][1], nextDots[0][1]])
        else:
            ...
            centers2 = [center[0] + calLength2 * math.cos(math.radians(
                rotateAngle2)), center[1] + calLength2 * math.sin(math.radians(rotateAngle2))]
            nextDots = drawDot(
                remainLiteralList[0], centers2, rotateAngle2, plt)
            plt.plot([thisDots[1][0], nextDots[0][0]],
                     [thisDots[1][1], nextDots[0][1]])
...
def drawPattern1(literalList, center):
    ...
    for literals in literalList:
        ...
        centers = [center[0] + literalsLengthHalf * math.cos(math.radians(
        ...
        if (i < len(literalList) - 1):
            ...
            rotateAngle2 = rotateRate * (i + 1)
            centers2 = [center[0] + literalsLengthHalf2 * math.cos(math.radians(
                rotateAngle2)), center[1] + literalsLengthHalf2 * math.sin(math.radians(rotateAngle2))]
            nextDots = drawDot(
                literalsList[i + 1], centers2, rotateAngle2, plt)
            plt.plot([thisDots[1][0], nextDots[0][0]],
                     [thisDots[1][1], nextDots[0][1]])
        else:
