...
class enterPage(QWidget):
    ...
    def initUI(self):
        ...
        canvasCSCub = FigureCanvas(overlapPatten.plt.gcf())
        self.cid = canvasCSCub.mpl_connect('button_press_event', self.show_fullScreen)
        """
        canvasCSCub = FigureCanvas(overlapPatten.plt.gcf())
        # 鼠标点击, 展示plt界面
        def show_plt(self):
        ...
        canvasCSCub.mpl_connect('button_press_event', show_plt)
        """
        canvasCSCirc = FigureCanvas(figure(facecolor='red'))
        # canvasCSCirc = FigureCanvas(overlapPatten.plt.gcf())
